...
function Assert-RequiredCommand {
    ...
    if ($null -eq (Get-Command $Name -ErrorAction SilentlyContinue)) {
        throw "HITO4_CLOSURE_REQUIRED_COMMAND_MISSING:$Name"
    }
}
...
function Assert-LocalSupabase {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'HITO4_CLOSURE_LOCAL_SUPABASE_NOT_RUNNING'
    }

    $status = $statusJson | ConvertFrom-Json
    $apiUrl = [Uri]$status.API_URL
    if ($apiUrl.Host -notin @('localhost', '127.0.0.1')) {
        throw 'HITO4_CLOSURE_REFUSES_NON_LOOPBACK_SUPABASE'
    }

    if ($apiUrl.Port -ne $expectedLocalApiPort) {
        throw "HITO4_CLOSURE_UNEXPECTED_SUPABASE_PORT:$($apiUrl.Port)"
    }
}
...
function Assert-RequiredFiles {
    ...
    foreach ($path in $Paths) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "HITO4_CLOSURE_REQUIRED_FILE_MISSING:$path"
        }
    }
}
...
function Invoke-CheckedCommand {
    ...
    Write-Output "HITO4_CLOSURE_STAGE=$Name"
    & $Command
    if ($LASTEXITCODE -ne 0) {
        throw "HITO4_CLOSURE_STAGE_FAILED:$Name"
    }
}
...
try {
    ...
    Assert-RequiredFiles -Paths @(
        '.\supabase\tests\database\hito4_private_history_test.sql',
        '.\supabase\tests\database\hito4_unanswered_operations_test.sql',
        '.\supabase\tests\database\hito4_security_and_user_admin_test.sql',
        '.\apps\api\src\chat\chat-history-cursor.spec.ts',
        '.\apps\api\src\operations\operations.service.spec.ts',
        '.\apps\api\src\user-administration\user-administration.service.spec.ts',
        '.\apps\api\src\supabase\supabase-user-administration.gateway.spec.ts'
    )
    ...
    Invoke-CheckedCommand -Name 'api-coverage-and-hito4-contracts' -Command {
        & npm run test:cov --workspace=api
    }
    Invoke-CheckedCommand -Name 'api-e2e-regression' -Command {
    ...
    Invoke-CheckedCommand -Name 'workspace-test-regression' -Command {
        & npm run test
    }
    ...
    Write-Output 'HITO4_LOCAL_ACCEPTANCE=PASS'
}
finally {
    Pop-Location
}

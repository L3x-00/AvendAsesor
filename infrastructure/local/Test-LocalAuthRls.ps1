...
try {
    ...
    if ($apiUrl.Host -notin @('localhost', '127.0.0.1')) {
        throw 'Local Auth/RLS regression refuses a non-loopback Supabase URL.'
    }
...
const emailPrefix = 'avend-local-auth-';
const email = `${emailPrefix}${Date.now()}@avend.local`;
const password = 'AvendLocal!2026Auth';
let stage = 'bootstrap';
...
async function waitForEmail(predicate) {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    for (const message of body.messages ?? []) {
      ...
      if (serialized.includes(email) && predicate(serialized)) return serialized;
    }
...
(async () => {
  ...
  try {
    ...
    const signup = await request(`${apiUrl}/auth/v1/signup`, {
      ...
      body: JSON.stringify({
        email,
        password,
        data: { full_name: 'Local Auth Regression' },
    ...
    const initialAccessToken = signup.body.session?.access_token ?? signup.body.access_token;
    if (!userId || initialAccessToken || signupUser.email_confirmed_at) throw new Error('UNEXPECTED_SIGNUP_RESPONSE');

    stage = 'confirmation-email';
    const confirmationMail = await waitForEmail((message) => message.toLowerCase().includes('confirm'));
    stage = 'confirmation-link';
    ...
    if (confirmationResponse.status < 300 || confirmationResponse.status > 399) throw new Error(`CONFIRMATION_STATUS_${confirmationResponse.status}`);
    ...
    stage = 'rls';
    const profile = await request(`${apiUrl}/rest/v1/profiles?select=id,role&id=eq.${userId}`, {
      headers: { apikey: anonKey, Authorization: `Bearer ${accessToken}` },
    });
    if (!Array.isArray(profile.body) || profile.body.length !== 1 || profile.body[0].role !== 'docente') {
      throw new Error('RLS_PROFILE_MISMATCH');
    }

    stage = 'recovery-request';
    await request(`${apiUrl}/auth/v1/recover`, {
      method: 'POST',
      headers: { apikey: anonKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, redirect_to: 'http://localhost:3000/auth/callback?next=/auth/update-password' }),
    });
    stage = 'recovery-email';
    await waitForEmail((message) => message.toLowerCase().includes('recover'));
    result = 'PASS';
  } catch (error) {
  ...
  console.log(`LOCAL_AUTH_RLS_REGRESSION=${result}${detail}`);

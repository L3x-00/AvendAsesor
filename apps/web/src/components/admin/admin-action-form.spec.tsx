...
describe('AdminActionForm', () => {
  it('blocks duplicate submission while an action is pending and shows a safe recovery message', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('alert')).toHaveTextContent('Actualiza el listado');
    });
  });
  ...
  it('renders a short-lived download URL only when returned by the server action', async () => {
    ...
    const action: AdminAction = async () => ({
      downloadUrl: 'http://localhost:55321/storage/v1/object/sign/normative-documents/test',
    ...
    expect(screen.getByRole('link', { name: 'Abrir descarga temporal' })).toHaveAttribute(
      'href',
